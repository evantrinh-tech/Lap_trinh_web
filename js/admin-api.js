...
// Load đơn hàng hoàn thành gần đây
async function loadRecentOrders() {
  ...
  try {
    const res = await adminFetch(
      `${ADMIN_API}/admin/index.php?section=recent_orders`,
    );
    const data = await res.json();
    ...
    tbody.innerHTML = data.data
      ...
      .map((order) => {
        ...
        // Xử lý created_at từ MongoDB (có thể là { $date: "..." } hoặc string)
        const rawDate = order.created_at?.$date ?? order.created_at;
        const date = rawDate
          ? new Date(rawDate).toLocaleDateString("vi-VN")
          : "";

        // MongoDB trả _id thay vì id
        const rawId = order._id || order.id || "";
        const id = "#KX-" + rawId.slice(-6).toUpperCase();
